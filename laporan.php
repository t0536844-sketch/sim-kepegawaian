...
<head>
    ...
    <style>
        ...
        .chart-container {
            position: relative;
            height: 250px; /* Fixed height untuk grafik */
            width: 100%;
            margin-bottom: 10px;
        }
        
        /* Style untuk kontainer grafik */
        .chart-wrapper {
            background: white;
            border-radius: 10px;
            padding: 15px;
...
<body>
            ...
            <div class="col-md-6">
                <div class="chart-wrapper">
                    ...
                    <div class="text-center mt-2">
                        <small class="text-muted">Total: <?php echo $stats['total']; ?> pegawai</small>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
    ...
    <script>
        ...
        statusChartCanvas.style.height = '250px';
        ...
        const statusChart = new Chart(statusCtx, {
            ...
            options: {
                responsive: true,
                maintainAspectRatio: false, // Penting: false untuk kontrol manual
                plugins: {
                    legend: {
                        position: 'bottom',
                        labels: {
        ...
        // Set fixed size for canvas
        genderChartCanvas.style.width = '100%';
        genderChartCanvas.style.height = '250px';
        ...
        function printReport() {
            ...
            const printContent = `
                ...
                <body>
                    ...
                    <div class="summary">
                        <strong>Ringkasan:</strong> Total ${<?php echo count($pegawai); ?>} data pegawai ditemukan
                    </div>
                                    ...
                                    <?php foreach ($pegawai as $row): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php
                                                $keluarga = [];
                                                if (!empty($row['nama_pasangan'])) $keluarga[] = htmlspecialchars($row['nama_pasangan']);
                                                if (!empty($row['jumlah_anak'])) $keluarga[] = $row['jumlah_anak'] . ' anak';
                                                echo !empty($keluarga) ? implode(', ', $keluarga) : '-';
                                                ?>
                                            </td>
                                            <td><?php echo !empty($row['alamat_rumah']) ? htmlspecialchars($row['alamat_rumah']) : '-'; ?></td>
                                            <td>
                                                <?php 
                                                if (!empty($row['masa_berlaku_str'])) {
                                                    echo date('d/m/Y', strtotime($row['masa_berlaku_str']));
                                                } else {
                                                    echo '-';
                                                }
                                                ?>
                                            ...
                                            <td>
                                                <?php 
                                                if (!empty($row['masa_berlaku_sip'])) {
                                                    echo date('d/m/Y', strtotime($row['masa_berlaku_sip']));
                                                } else {
                                                    echo '-';
                                                }
                                                ?>
        ...
        function exportToPDF() {
            alert('Untuk export PDF, gunakan fitur "Cetak Laporan" lalu pilih "Microsoft Print to PDF" atau "Save as PDF" pada printer options.\n\nAtau gunakan library jsPDF dengan menginstal melalui CDN.');
        }
        ...
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            if (countElement) {
                countElement.innerHTML = `Data Pegawai (${visibleCount} data)`;
            }
        });
    ...
    <style media="print">
        @media print {
            ...
            .filter-card h5:first-child, .chart-wrapper,
            .row.mt-4, .input-group {
            ...
            .table thead th {
                ...
                position: static !important; /* Remove sticky on print */
            }
            .table td {
                padding: 4px !important;
                border: 1px solid #ddd !important;
